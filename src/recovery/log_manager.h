...
public:
    ...
    InsertLogRecord(txn_id_t txn_id, RmRecord& insert_value, const Rid& rid, std::string table_name) 
        : InsertLogRecord() {
        log_tid_ = txn_id;
...
public:
    ...
    DeleteLogRecord(txn_id_t txn_id, RmRecord& delete_value, const Rid& rid, std::string table_name) 
        : DeleteLogRecord() {
        log_tid_ = txn_id;
...
public:
    ...
    UpdateLogRecord(txn_id_t txn_id, RmRecord& old_value, RmRecord& new_value, const Rid& rid, std::string table_name) 
        ...
        rid_ = rid;
        log_tot_len_ += sizeof(int) * 2;  // old_value size + new_value size
...
public:
    ...
    void force_flush_log_to_disk(lsn_t lsn);  // WAL机制：强制刷新日志到磁盘
    lsn_t get_persist_lsn() const;            // 获取当前持久化的LSN
    lsn_t get_global_lsn() const;             // 获取当前全局LSN
