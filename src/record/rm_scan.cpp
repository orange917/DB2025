...
RmScan::RmScan(const RmFileHandle *file_handle) : file_handle_(file_handle) {
    ...
    //         rid_.slot_no=Bitmap::first_bit(true, page_handle.bitmap, file_handle_->file_hdr_.num_records_per_page);
    //         rid_.page_no=i;
    //         file_handle_->buffer_pool_manager_->unpin_page({.fd=file_handle_->fd_,.page_no=i},false);
    //         break;
    //     }
    //     file_handle_->buffer_pool_manager_->unpin_page({.fd=file_handle_->fd_,.page_no=i},false);
    // }
...
void RmScan::next() {
    ...
    while (rid_.page_no != RM_NO_PAGE && rid_.page_no < file_handle_->file_hdr_.num_pages) {
        ...
        file_handle_->buffer_pool_manager_->unpin_page({.fd=file_handle_->fd_, .page_no=rid_.page_no}, false);
