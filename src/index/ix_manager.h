...
class IxManager {
   ...
   public:
    ...
    std::unique_ptr<IxIndexHandle> open_index(const std::string &filename, const std::vector<ColMeta>& index_cols, const IndexMeta* meta = nullptr) {
        std::string ix_name = get_index_name(filename, index_cols);
        int fd = disk_manager_->open_file(ix_name);
        if (meta) {
            return std::make_unique<IxIndexHandle>(disk_manager_, buffer_pool_manager_, fd, *meta);
        } else {
